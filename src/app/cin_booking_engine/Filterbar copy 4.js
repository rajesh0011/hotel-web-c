...
const guardedFetch = () => {
  if (!hasFetched) {
    ...
    // reset guard after short delay (so future searches still work)
    setTimeout(() => setHasFetched(false), 300);
  }
};
...
useEffect(() => {
 ...
 // const [defaultOffer, setDefaultOffer] = useState(null);
  ...
  useEffect(() => {
   ...
   useEffect(() => {
     if (!isLoaderOverlay && hasSearched) {
      postBookingWidged("","", false,"rate Fetched","rate","https://cinbe.cinuniverse.com/api/cin-api/rate",);
     }
   }, [isLoaderOverlay]);
  // useEffect(() => {
  ...
  let apiErrorCodeData = "";  
  let apiMessageData = "";
  let ctaNameData = "Fetch Rooms Rates"; 
  ...
  let apiUrlData = "https://clarkscms.cinuniverse.com/Api/gallery/GetGalleryByProperty";  
    try {
  ...
  let apiUrlData = "https://cinbe.cinuniverse.com/api/verify-token"; 
    try {
...
const fetchRatePrices = async (propertyId, defaultOffer) => {
  ...
  let apiUrlData = "https://cinbe.cinuniverse.com/api/cin-api/rate"; 
  if (!propertyId) {
    setFilteredProperties([]);
  ...
  // "rate","https://cinbe.cinuniverse.com/api/cin-api/rate",response?.statusText,response?.status,"Data not found")
  //   }, 200);
      //throw new Error("Failed - Rate not found.");
    ...
    if (contentProperties?.[0]?.RoomData && dayRate?.length > 0) {
     ...
     // Step 1: Get all rooms with MinInventory > 0
      ...
      // setTimeout(() => {
      //       postBookingWidged("","", false,"Fetch Rate", 
      //   "rate","https://cinbe.cinuniverse.com/api/cin-api/rate",response?.status,response?.status,"Data found")
      //     }, 200);
      } else {
...
//   }
  ...
  let apiUrl = "https://clarkscms.cinuniverse.com/Api/property/GetBrandList"; 
    ...
    .finally(() => {
      setTimeout(() => {
      postBookingWidged(roomsData,mappingData, isClose,ctaName, 
  ...
  let apiUrlData = "https://cinbe.cinuniverse.com/api/cin-api/add-ons";  
      try {
  ...
  let apiUrlData = `https://clarkscms.cinuniverse.com/Api/property/GetCityWithProperty?CityId=${parseInt(
              cityName?.value
            )}`; 
 ...
 async function postBookingWidged(rooms,mapping, isClose,ctaName, 
  ...
  // }
    ...
    propertyId: selectedPropertyId ? selectedPropertyId?.toString() : "0",
    checkIn: fromDate ?? "",
    checkOut: toDate ?? "",
  ...
  setSelectedRoom(resetRooms);
    ...
    // } 
      ...
      // if(selectedProperty == 0 || (isPromoCodeChanged && selectedProperty > 0) || (isDateChanged && selectedProperty > 0) ){
      //     fetchRatePrices(selectedPropertyId,defaultOffer);
      //     setIsDateChanged(false);
      //     setPromoCodeChanged(false);
      // }
      if (
    selectedProperty == 0 ||
    (!showDownDiv  && selectedProperty > 0) ||
    (isPromoCodeChanged && selectedProperty > 0) ||
    (isDateChanged && selectedProperty > 0)
  ) {
    guardedFetch();
  }
      if(!showDropDown){
...
//  fetchAddOns();
 ...
 // postBookingWidged(rooms, mapping, false);
  ...
  useEffect(() => {
   ...
   useEffect(() => {
     if ((showDownDiv  && selectedProperty > 0) ||(contentProperties?.length > 0 && selectedProperty > 0)) {
       //fetchRatePrices(selectedPropertyId,defaultOffer);
        guardedFetch();
     }
   }, [contentProperties,showDownDiv]);
        ...
        <div className="row g-0 mb-3" key={rooms?.RoomId}>
                   ...
                   :(
                     <div className="no-property">
                       {isRateFetched && showDropDown ? <><div className="repeated-div-property"><p>Apologies, we couldn’t find any properties matching your search.</p></div></>:''}
                     </div>
                   ) 
                  }
                </div>
